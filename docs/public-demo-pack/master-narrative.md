...
| GREEN | The white paper baseline lived in the Aurora strategy pack outside the repo, unversioned. | Closed by SF-279. | Versioned at `docs/product/2026-07-01-product-white-paper-baseline.md`, body byte-identical to the source. Deliberately preserved as a dated historical baseline rather than imported as a current description: its roadmap, control-board summary and next-actions were all stale, and it used "Simulation Patient Twin" as an alternate product name — the exact competing label SF-275 removed. A header states it is superseded, points here as the controlled source, and lists what has materially changed. Its safety boundary section had held unchanged and is still accurate. |
