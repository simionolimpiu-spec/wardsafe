...
| SF-102 | In progress | Product concept | Patient Journey Twin / Simulation Patient Twin concept rescooped into PR #20 (`feature/patient-journey-twin-v2`). | Branch is pushed and review-ready on top of the latest `origin/codex/safeflow-prototype`, uses the shared `src/domain/patientTimeline.js` / `src/data/patientTimelineFixtures.js` model, and drops the duplicate branch-local timeline module plus the accidental tracker/roadmap edits. PR #14 remains superseded/closed. |
