...
| Current branch | docs/aws-mock-architecture (docs-only AWS mock architecture branch in progress). |
...
| SF-104 | In progress | AWS architecture | Mock/readiness AWS direction documented in `docs/public-demo-pack/build-readiness/architecture-options.md` and `docs/public-demo-pack/build-readiness/technology-stack.md` (Aurora, Step Functions, Bedrock/LLM draft provider, tokenised backend, RBAC/audit logging). | Keep as mock/readiness architecture only; no live AWS deployment or SDK wiring until explicit approval. |
